...
def get_user_pages(users):

	pages = []

	for user in users:
		pages.append([user.get_name(), user.on_page()])
...
def next_doodle(users, doodles):

	# initial doodles that show instructions
	if get_round() == 0:
		return [initial_image, initial_text]

	from_user = getting_from(users)

	for doodle in doodles:
		if doodle.get_name() == from_user and doodle.get_round() == get_round()-1:
			return [doodle.get_imageURL(), doodle.get_text()]

	
def getting_from(users):

	index = 0
	for user in users:
		if user.get_name() == session['username']:
			index = user.get_order()

	if index == 0:
		index = len(users)

	for user in users:
		if user.get_order() == index-1:
...
class Doodle:
	...
	def __init__(self, name, round, image, text):
		self.game_id = 1
		self.round = round
		self.user_name = name
		self.image = image
...
def playing():
	...
	if request.method == 'GET':
		...
		print("Updating doodle.")
		send_doodle = next_doodle(users, doodles)

		print("Next doodle text: %s" %send_doodle[1])

		set_page(users, 'playing')
		return render_template('playing.html', showDoodle=send_doodle)
	...
	if request.method == 'POST':
		...
		new_doodle = Doodle(session['username'], get_round(), image, text)
		doodles.append(new_doodle)

		set_page(users, 'waiting')
...
def waiting():
	...
	if all_on_page(users, "waiting"):
		print("Everyone is done waiting! Round %i." %get_round())
		next_round()
		return redirect(url_for('playing'))
	else:
		print("Still waiting...")
